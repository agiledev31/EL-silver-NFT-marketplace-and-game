...
@Component({
  selector: 'app-exchanedefault',
  templateUrl: './exchangedefault.component.html',
  styleUrls: ['./exchangedefault.component.css'],
})
export class ExchangeDefaultComponent implements OnInit { 
  isLoader = false;
  exchangeOrder = true
  fromAmount: any = ''
  toAmount: any = ''
  ...
  async initLPLT(){
    ...
    this.bnbBalance = await this.web3Service.Get_BNB_Balance(this.web3Service.getConnectedAccount());
    this.lpltBalance = await this.web3Service.Get_LPLT_Balance(this.web3Service.getConnectedAccount());
    this.bnbBalance = this.web3Service.getPrice(1, this.bnbBalance, false );
    this.lpltBalance = this.web3Service.getPrice(1, this.lpltBalance, false );
  }
